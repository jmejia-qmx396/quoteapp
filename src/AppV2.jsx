...
const ProductsView = ({ products, setProducts }) => {
  ...
  return (
    <div style={{ padding:30 }}>
      ...
      <Card style={{ padding:0,overflow:"hidden" }}>
        <table>
          <thead><tr><th>SKU</th><th>Producto / Servicio</th><th>Categoría</th><th>Costo</th><th>Margen</th><th>P. Venta</th><th>Unidad</th><th></th></tr></thead>
          <tbody>
            {filt.map(p=>(
              <tr key={p.id}>
                ...
                <td style={{ fontFamily:G.mono,color:G.muted }}>{fmt(p.cost||0)}</td>
                <td><span className="badge badge-warn">{p.margin||0}%</span></td>
                <td style={{ fontFamily:G.mono,fontWeight:700,color:G.success }}>{fmt(p.price)}</td>
            ...
            {!filt.length && <tr><td colSpan={8} style={{ textAlign:"center",color:G.muted,padding:24 }}>Sin productos.</td></tr>}
          </tbody>
        </table>
      </Card>

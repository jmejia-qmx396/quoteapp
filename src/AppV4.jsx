...
const recalc = (q) => {
  ...
  // priceCOP = precio en COP (si el item es USD, se multiplica por TRM)
  const items = q.items.map(i => ({
    ...i,
    priceCOP: i.currency === "USD" ? Number(i.price) * trm : Number(i.price),
    costCOP:  i.currency === "USD" ? Number(i.cost||0) * trm : Number(i.cost||0),
  }));
  ...
  const totalSale  = subtotal - disc + taxAmt;
  const profit     = totalSale - totalCost - (totalCost > 0 ? disc : 0);
  const profitPct  = totalSale > 0 ? Math.round((profit / totalSale) * 100) : 0;
  return { ...q, items, subtotal, discountAmt: disc, taxAmt, total: totalSale, totalCost, profit, profitPct };
};
  ...
  return (
    <Modal title={isNew ? "Nueva Cotización" : `Editar Cotización #${quote.number}`}
      ...
      <Card style={{ padding:0,overflow:"hidden",marginBottom:18 }}>
        <table>
          <thead><tr>
            <th>SKU</th><th>Descripción</th><th style={{width:60}}>Moneda</th>
            <th style={{width:80}}>Qty</th><th style={{width:130}}>P. Unitario</th>
            <th>Total COP</th>
          ...
          <tbody>
            {quote.items.map(item=>{
              const lineTotalCOP = Number(item.qty) * (item.priceCOP||Number(item.price));
              const lineCostCOP  = Number(item.qty) * (item.costCOP||Number(item.cost||0));
              const lineProfit   = lineTotalCOP - lineCostCOP;
              const linePct      = lineTotalCOP > 0 ? Math.round((lineProfit/lineTotalCOP)*100) : 0;
              return (
              <tr key={item.id}>
                ...
                <td>
                  <div style={{ display:"flex",alignItems:"center",gap:4 }}>
                    ...
                    {item.currency==="USD" && (
                      <span style={{ fontSize:10,color:G.muted,whiteSpace:"nowrap" }}>
                        ={fmt((item.priceCOP||0))}
                      </span>
                    )}
                  </div>
            ...
            {!quote.items.length && (
              <tr><td colSpan={8} style={{ textAlign:"center",color:G.muted,padding:24,fontStyle:"italic" }}>
      ...
      <div style={{ display:"flex",gap:14,justifyContent:"flex-end",marginBottom:18 }}>
        ...
        <div style={{ width:240,background:"rgba(16,185,129,.06)",border:"1px solid rgba(16,185,129,.2)",
          ...
          {[["Costo total",fmt(quote.totalCost||0)],
            ["Venta neta",fmt((quote.subtotal||0)-(quote.discountAmt||0))],
        ...
        <table>
          ...
          <tbody>
            ${quote.items.map((it,i)=>{
              ...
              const totalCOP = Number(it.qty) * priceCOP;
              return `<tr>
                <td style="color:#94a3b8">${i+1}</td>
                <td><code>${it.sku}</code></td>
                <td>${it.name}</td>
                <td style="text-align:center">${it.qty} ${it.unit||""}</td>
                <td style="text-align:right">${fmtCOP(priceCOP)}</td>
                <td style="text-align:right;font-weight:600">${fmtCOP(totalCOP)}</td>
              </tr>`;
            }).join("")}
          </tbody>
          <tfoot>
            ...
            ${(quote.discount>0)?`<tr><td colspan="5" style="text-align:right;color:#ef4444;padding:6px 12px">- Descuento (${quote.discount}%)</td><td style="text-align:right;color:#ef4444;padding:6px 12px">-${fmtCOP(quote.discountAmt||0)}</td></tr>`:""}
